...
@property (nonatomic, readonly, strong) EMBEndpoints * _Nullable endpoints;
...
/// Initializer for <code>Embrace.Options</code> that does not require an appId.
...
/// \param logLevel The <code>LogLevel</code> to use for console logs.
///
- (nonnull instancetype)initWithExport:(EMBOpenTelemetryExport * _Nonnull)export_ platform:(enum EMBPlatform)platform captureServices:(NSArray<EMBCaptureService *> * _Nonnull)captureServices crashReporter:(id <CrashReporter> _Nullable)crashReporter logLevel:(enum LogLevel)logLevel OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// important:
/// Throws an exception if no proper <code>CrashReporter</code> is configured.
/// \param key The key for the attribute.
///
/// \param value The value associated with the given key.
///
- (BOOL)appendCrashInfoWithKey:(NSString * _Nonnull)key value:(NSString * _Nonnull)value error:(NSError * _Nullable * _Nullable)error;
